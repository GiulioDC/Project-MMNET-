...
//Rare exceptions to indicate a bug in the _idea code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_idea = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_idea]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.idea.module"
